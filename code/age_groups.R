...
ssi_filer <-  read_csv("../data/ssifiler.txt", col_names = FALSE)
...
ssi_filer <- ssi_filer[!(ssi_filer$date=="200728"),]
ssi_filer <- ssi_filer[!(ssi_filer$date=="200622"),]
...
muni_df %>% 
  mutate(pos_temp = ifelse(`Antal_bekræftede_COVID-19` == "<10", 1, `Antal_bekræftede_COVID-19`)) %>%
  mutate(pos_temp = as.numeric(gsub("\\.", "", pos_temp))) %>%
  mutate(Date = as.Date(Date)) %>%
  select(-date_of_file) %>%
  arrange(Date)
...
# Combine AGE, ADMITTED and play with data --------------------------------
...
ggplot(plot_data, aes(Date, value)) + 
  ...
  scale_fill_discrete(name = "Variabel", labels = c("Pos over 50 år", "Nyindlagte")) + 
  labs(y = "Antal", x = "Dato") + 
...
ggplot(plot_data, aes(Date, value)) + 
  geom_bar(stat = "identity", position = "stack", aes(fill = variable)) + 
  scale_fill_discrete(name = "Variabel", labels = c("Pos under 50 år", "Nyindlagte")) + 
  labs(y = "Antal", x = "Dato") + 
  scale_y_continuous(breaks = c(-500,0, 500, 1000),labels=as.character(c("500","0", "500", "1000"))) +
  theme_minimal() + 
  theme(text = element_text(size=14, family="lato"),
...
ggplot(plot_data, aes(Date, value)) + 
  ...
  labs(y = "Andel positive tests", x = "Dato") + 
  theme_minimal() + 
  theme(text = element_text(size=14, family="lato"),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
        axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0)))
...
ggsave("../figures/age_group_admitted_pos_old.png",width = 20, height = 12, units = "cm", dpi = 300)
